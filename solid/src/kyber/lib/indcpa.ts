...
export class Indcpa {
  ...
  /**
   * Generate two keypairs to PQ encrypt the wallet password.
   * @param rcvSeed Seed to generate key with. This is the password in KaspX
   * @returns Private key used for PKE
   */
  public indcpaKeyGenWithSeed (rcvSeed: number[]): number[][] {
    // hash received seed with SHA3-512
    const buffer1 = Buffer.from(rcvSeed)
  ...
  public indcpaEncrypt (publicKey: number[], msg: number[], coins: number[]): number[] {
    ...
    let end
    // decode message m
    const k = this.poly.polyFromData(msg)
    for (let i = 0; i < this.paramsK; i++) {
    ...
    const epp: number[] = this.poly.getNoisePoly(coins, (this.paramsK * 2), 3)
  ...
  public generateMatrix (seed: number[], transposed: boolean): number[][][] {
    const a: number[][][] = [] // this.paramsK)
    const xof = new SHAKE(128)
    let ctr = 0
    for (let i = 0; i < this.paramsK; i++) {
      a[i] = [] // this.paramsK
      const transpose: number[] = [] // 2
      for (let j = 0; j < this.paramsK; j++) {
        ...
        const outputString = xof.digest({ format: 'binary', buffer: Buffer.alloc(672) })
        const output = Buffer.alloc(outputString.length)

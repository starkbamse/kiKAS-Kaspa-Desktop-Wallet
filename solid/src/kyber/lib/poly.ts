...
export class Poly {
  ...
  /**
     ...
     * @return
     */
  public polyFromData (msg: number[]): number[] {
    const r: number[] = []
    for (let i = 0; i < KyberService.paramsPolyBytes; ++i) {
      r[i] = 0
    }
  ...
  public polyToMont (r: number[]): number[] {
    for (let i = 0; i < KyberService.paramsN; i++) {
      r[i] = this.byteOps.byteopsMontgomeryReduce(int32(r[i]) * int32(1353))
    }
    return r
  }
  ...
  /**
     ...
     * @return
     */
  public nttBaseMuliplier (a0: number, a1: number, b0: number, b1: number, zeta: number): number[] {
    const r: number[] = [] // 2
